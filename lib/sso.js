...
module.exports = {
    ...
    authenticate: function authenticate(req, res, next, strategy) {
        ...
        if (res.locals.connected) { // Forward to endpoint
            ...
            const loginString = strategy.validator(req, req.session.user);
            ...
            req.flash('info', `Log in with your OpenMRS ID to access ${strategy.serviceName}. If you don't have one, please sign up  <a href="/signup">here</a>.`);

          const search = route.search || ''; // Avoid undefined
          const uriComponents = encodeURIComponent(route.pathname + search);
            return res.redirect(`/login?destination=${uriComponents}`);
        }
    },
