...
app.get('/globalnav/admin', mid.restrictTo('dashboard-administrators'), function(req, res, next){
	db.getAll('GlobalNavLinks', function(err, links){
		res.render(__dirname+'/../views/admin', {links: links});
	});
});

app.post('/globalnav/admin', mid.restrictTo('dashboard-administrators'), function(req, res, next){
	// form body to array
	var generatedList = [];
	for (a in req.body) {
